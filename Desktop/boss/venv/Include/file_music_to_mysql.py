    ...
    with open('C:\\Users\\lch\\Desktop\\json\\music', encoding='utf8') as f:
        ...
        while True:
            ...
            try:
                ...
                result = []
                result.append((str(review_text['singer_name']).replace("[","").replace("]","").replace("'",""),
                               review_text['song_name'], review_text['subtitle'],
                               review_text['album_name'],
                               str(review_text['singer_id']).replace("[","").replace("]",""),
                               str(review_text['singer_mid']).replace("['","").replace("']","").replace("'",""),
                               review_text['song_time_public'],
                               review_text['song_type'],
                               review_text['language'],
                               review_text['song_id'],
                               review_text['song_mid'],
                               review_text['song_url'],
                               str(review_text['hot_comments']),
                               review_text['lyric']))
...
if __name__ == "__main__":  # 起到一个初始化或者调用函数的作用
    db = pymysql.connect("192.168.75.129", "root", "123456", "music", charset='utf8mb4')
    cursor = db.cursor()
    prem(db)
